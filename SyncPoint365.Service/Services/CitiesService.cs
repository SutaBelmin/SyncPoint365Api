...
namespace SyncPoint365.Service.Services
{
    public class CitiesService : BaseService<City, CityDTO, CityAddDTO, CityUpdateDTO>, ICitiesService
    {
        ...
        public async Task<IPagedList<CityDTO>> GetPagedCitiesAsync(int? countryId = null, string? query = null, int page = Constants.Pagination.PageNumber, int pageSize = Constants.Pagination.PageSize, bool isAscending = true, CancellationToken cancellationToken = default)
        {
            var pagedList = await _repository.GetPagedCitiesAsync(countryId, query, page, pageSize, isAscending, cancellationToken);
