...
namespace SyncPoint365.API.Controllers
{
    ...
    public class CitiesController : BaseController<CityDTO, CityAddDTO, CityUpdateDTO>
    {
        ...
        public async Task<IActionResult> GetPagedCitiesAsync(int? countryId = null, string? query = null, int page = Constants.Pagination.PageNumber, int pageSize = Constants.Pagination.PageSize, bool isAscending = true, CancellationToken cancellationToken = default)
        {
            var data = await _citiesService.GetPagedCitiesAsync(countryId, query, page, pageSize, isAscending, cancellationToken);
